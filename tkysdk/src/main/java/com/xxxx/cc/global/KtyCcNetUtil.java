                            ...
                            try {
                                ...
                                if (baseBean != null) {
                                    if (baseBean.isOk()) {
                                        ...
                                        loginCallBack.onSuccess(ErrorCode.SUCCESS, "登录成功",null);
                                        //判断是否已经启动service了
//                                        dealLinkLinPhone(context,baseBean,loginCallBack);
    ...
    public static void loginBySdk(Context context, String userName, String pwd, LoginCallBack loginCallBack) {
        if (loginCallBack == null) {
           LogUtils.e("loginCallBack 不能为null");
            return;
        }
        //判断网络是否可用
        if (!NetUtil.isNetworkConnected(context)) {
                                ...
                                if (baseBean != null) {
                                    if (baseBean.isOk()) {
                                        try {
                                            ...
                                            loginCallBack.onSuccess(ErrorCode.SUCCESS, "登录成功",userBean);
//                                            dealLinkLinPhone(context,baseBean,loginCallBack);
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                            loginCallBack.onFailed(ErrorCode.INTERNAL_SERVER_ERROR, e.getMessage());
                                        }
//                                        dealLoginSuccess(context, baseBean);
                                    ...
                                    loginCallBack.onFailed(ErrorCode.INTERNAL_SERVER_ERROR, "服务器错误1");
                                }
                            } catch (Exception e) {
                                loginCallBack.onFailed(ErrorCode.INTERNAL_SERVER_ERROR, "服务器错误2");
                            }
                        } else {
                            loginCallBack.onFailed(ErrorCode.INTERNAL_SERVER_ERROR, "服务器错误3");
                        }
                    }
                });
    }
